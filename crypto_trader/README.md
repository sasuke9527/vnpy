...
> **真实数据回测结论（2026-09-25）见 [RESEARCH.md](RESEARCH.md)。** 要点：S2 全线亏损已弃用；S1 在 ETH 上 2025 至 2026 年亏损；
> 唯一稳定为正的形态是"meme 币只做多 + 25 日趋势门控"，`settings.DEPLOYMENT` 的默认已改为 1000PEPE + 1000SHIB + WIF 三个仓袋。
> 该配置**仍未通过第 7 节验收门槛**（验证期夏普 0.6 到 0.66），45 个月里最好的单月 +118%，从未出现过 +500% 的月份。
> 下面第 1 节的"预期收益"表是设计阶段的估计，以 RESEARCH.md 的实测为准。
